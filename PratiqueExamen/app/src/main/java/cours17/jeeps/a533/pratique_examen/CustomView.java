...
public class CustomView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawCircle(circle1X, 175, 20, myPaint);
        if(numberOfInput > 0){
            canvas.drawCircle(circle1X, 175, 20, circlePaint);
        }

        canvas.drawCircle(circle2X, 175, 20, myPaint);
        if(numberOfInput > 1){
            canvas.drawCircle(circle2X, 175, 20, circlePaint);
        }

        canvas.drawCircle(circle3X, 175, 20, myPaint);
        if(numberOfInput > 2){
            canvas.drawCircle(circle3X, 175, 20, circlePaint);
        }

        canvas.drawCircle(circle4X, 175, 20, myPaint);
        if(numberOfInput > 3){
